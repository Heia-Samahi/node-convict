...
convict.addParser = function(parsers) {
  ...
  /**
   * Auto generates from a raw schema the args and env tags.
   */
  convict.compileSchema = (def, generateArgs, generateEnv) => {
    const d = cloneDeep(def);
    const compileNode = (node, path, generateArgs, generateEnv) => {
      if (typeof node === 'object' && node !== null && !Array.isArray(
          node) &&
        Object.keys(node).length > 0 && !('default' in node)) {

        Object.keys(node).forEach((k) => {
          const p = path + '#####' + k;
          compileNode(node[k], p, generateArgs, generateEnv);
        });
        return;
      } else {
        if (generateArgs && generateArgs !== null && (!node.arg || node.arg ===
            null)) {
          if (!generateArgs.spacer || generateArgs.spacer === null) {
            generateArgs.spacer = '-';
          }
          node.arg = path.replace(/#####/g, generateArgs.spacer);
        }
        if (generateEnv && generateEnv !== null && (!node.env || node.env ===
            null)) {
          if (!generateEnv.spacer || generateEnv.spacer === null) {
            generateEnv.spacer = '_';
          }
          let env = path.replace(/#####/g, generateEnv.spacer);
          if (generateEnv.allCaps) {
            env = env.toUpperCase();
          }
          node.env = env;
        }
    ...
    Object.keys(d).forEach((k) => {
      compileNode(d[k], k, generateArgs, generateEnv);
    });
    return d;
  }
  ...
  /**
   * Generate the default config which can be provided to the application as a helper for the user.
   */
  convict.getDefaultConfig = (schema) => {
    const dc = {};

    const walkThroughNode = (node, dc) => {
      if (typeof node === 'object' && node !== null && !Array.isArray(
          node) &&
        Object.keys(node).length > 0 && !('default' in node)) {
